...
def main():
    ...
    for n_i in range(CONST_NUM_OF_TEST_DATA):
        ...
        (class_id, bounding_box, confidence) = Lib.Utilities.Image_Processing.YOLO_ONNX_Format_Object_Detection(image_data, model, [640, 480], 0.80)
        ...
        if class_id != None:
            for i, (class_id_i, bounding_box_i, confidence_i) in enumerate(zip(class_id, bounding_box, confidence)):
                ...
                Bounding_Box_Properties = {'Name': f'{CONST_OBJECT_NAME[class_id_i]}_{i}', 'Precision': f'{str(confidence_i)[0:5]}', 
                                            'Data': bounding_box_i}
